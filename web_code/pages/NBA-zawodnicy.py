...
class BasketballPlayers:
    ...
    def show_players(self, players_df):
        ...
        for _, player in players_df.iterrows():
            ...
            state_key = f"nba_player_state_{player['id']}"
            ...
            if not st.session_state[state_key]:
                if st.button(f"▶ {button_label}", key=f"nba_show_player_{player['id']}", use_container_width=True):
                    st.session_state[state_key] = True
                    st.rerun()
            else:
                if st.button(f"▼ {button_label}", key=f"nba_hide_player_{player['id']}", use_container_width=True):
                    st.session_state[state_key] = False
                    st.rerun()
