    ...
    def matches_buttons(self, filtered_matches):
        for _, row in filtered_matches.iterrows():
            ...
            if not st.session_state[state_key]:
                ...
                if st.button(f"▼ {button_label}", use_container_width=True, key=f"nba_hide_{row.id}"):
                    st.session_state[state_key] = False
                    st.rerun()
    ...
    def generate_teams_tab(self):
        ...
        with st.expander("Zespoły w sezonie {}".format(self.selected_season)):
            ...
            for key, value in self.teams_dict.items():
                ...
                if not st.session_state[state_key]:
                    if st.button(f"▶ {value}", use_container_width=True, key=f"nba_show_team_{key}"):
                        st.session_state[state_key] = True
                        st.rerun()
                else:
                    if st.button(f"▼ {value}", use_container_width=True, key=f"nba_hide_team_{key}"):
                        st.session_state[state_key] = False
                        st.rerun()
