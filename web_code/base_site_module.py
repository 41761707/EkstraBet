...
class Base:
    ...
    def set_config(self) -> None:
        ...
        # Konfiguracja głównych parametrów w układzie kolumnowym
        ...
        # Wybór zakresu danych (sezon/kolejka/daty)
        ...
        with col6:
            ...
            if self.filter_by_date:
                self.date_range = self.set_date_range()
        ...
        # Wybór modeli dla rodzin predykcji
    ...
    def show_predictions(self, home_goals, away_goals, id, result) -> None:
        ...
        if predictions is None:
            st.warning("Na chwilę obecną brak przewidywań dla wskazanego spotkania")
    ...
    def generate_match_button(self, row):
        ...
        # Jeśli szczegóły są ukryte - pokaż przycisk "Pokaż"
        if not st.session_state[state_key]:
            if st.button(f"▶ {button_label}", use_container_width=True, key=f"show_{row.id}"):
                st.session_state[state_key] = True
                st.rerun()
        else:
            # Jeśli szczegóły są widoczne - pokaż przycisk "Ukryj" i treść
            if st.button(f"▼ {button_label}", use_container_width=True, key=f"hide_{row.id}"):
                st.session_state[state_key] = False
                st.rerun()
    ...
    def show_teams(self, teams_dict: dict) -> None:
        ...
        for team_id, team_name in teams_dict.items():
            ...
            # Jeśli szczegóły są ukryte - pokaż przycisk "Pokaż"
            if not st.session_state[state_key]:
                if st.button(f"▶ {team_name}", use_container_width=True, key=f"show_team_{team_id}"):
                    st.session_state[state_key] = True
                    st.rerun()
            else:
                # Jeśli szczegóły są widoczne - pokaż przycisk "Ukryj" i treść
                if st.button(f"▼ {team_name}", use_container_width=True, key=f"hide_team_{team_id}"):
                    st.session_state[state_key] = False
                    st.rerun()
