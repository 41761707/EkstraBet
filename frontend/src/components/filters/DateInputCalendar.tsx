...
const FOOTER_BUTTON_CLASS_NAME =
  "rounded-md px-2 py-1 text-xs hover:bg-surface-muted " +
...
export function DateInputCalendar({
  ...
  return (
    <div
      ...
      className={
        "absolute z-50 mt-1 w-72 rounded-xl border border-border " +
    ...
    >
      ...
      <div className="mt-3 flex items-center justify-between">
        <button
          ...
          className={`${FOOTER_BUTTON_CLASS_NAME} text-accent-text`}
        >
          Dziś
        </button>
        {allowEmpty ? (
          <button
            ...
            onClick={onClear}
            className={`${FOOTER_BUTTON_CLASS_NAME} text-muted`}
          >
            Wyczyść
          </button>
        ) : null}
...
function CalendarHeader({ view, onViewChange }: CalendarHeaderProps) {
  return (
    <div className="flex items-center justify-between gap-1">
      ...
      <p className="min-w-0 flex-1 text-center text-sm font-medium text-text">
...
function dayButtonClassName(options: {
  ...
  const classes = ["h-8 cursor-pointer rounded-md text-sm transition"];
  if (options.isSelected) {
    classes.push("bg-accent text-on-accent");
  } else {
    classes.push(options.inCurrentMonth ? "text-text" : "text-subtle");
    if (!options.isDisabled) {
      classes.push("hover:bg-surface-muted");
    }
  }
  if (options.isToday && !options.isSelected) {
    classes.push("ring-1 ring-accent-text");
  }
  if (options.isDisabled) {
