...
def find_player_id_by_name(first_name, last_name, team_ids, conn):
    ...
    WHERE first_name = %s AND last_name = %s AND current_club IN (%s, %s)
    LIMIT 1"""
    cursor.execute(query, (first_name, last_name, team_ids[0], team_ids[1]))
    result = cursor.fetchone()
    cursor.close()
    if result:
        return result[0]
...
def get_player_sog(driver, match_id, team_ids, conn):
    player_sog = driver.find_elements(By.CSS_SELECTOR, "div[data-id='Celne strzały zawodnika (z dogrywką) 230023']")[0].text
