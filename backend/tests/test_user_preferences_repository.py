...
class TestFetchPreferences(unittest.TestCase):
    ...
    @patch(_GET_CONN)
    def test_query_filters_user_and_returns_theme(
            self,
        ...
        self.assertIn("SELECT theme", query)
...
class TestUpsertTheme(unittest.TestCase):
    """INSERT must update only theme, commit, then return the stored row."""
    ...
    def test_upserts_theme_only_and_commits(
        ...
        conn, cursor = _mock_connection(
            mock_get_conn,
            row={"theme": "dark"})
        document = repo.upsert_theme(7, "dark")
        self.assertEqual(document, {"theme": "dark"})
        insert_query, insert_params = cursor.execute.call_args_list[0].args
        self.assertIn("INSERT INTO user_preferences", insert_query)
        self.assertIn("VALUES (%s, %s)", insert_query)
        self.assertIn("ON DUPLICATE KEY UPDATE", insert_query)
        self.assertIn("theme = VALUES(theme)", insert_query)
        self.assertEqual(insert_params, (7, "dark"))
        select_query, select_params = cursor.execute.call_args_list[1].args
        self.assertIn("SELECT theme", select_query)
        self.assertIn("FROM user_preferences", select_query)
