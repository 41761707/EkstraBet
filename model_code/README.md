...
- **Centralizacja parametrów systemów rankingowych przez klasę `RatingParameters`**

#### Klasa `RatingParameters`
Klasa odpowiedzialna za zbieranie i zarządzanie wszystkimi parametrami wymaganymi przez systemy rankingowe.

**Główne zadania:**
- Centralizuje pobieranie parametrów z konfiguracji modelu
- Zapewnia bezpieczne odczytywanie wartości z wartościami domyślnymi
- Buduje atrybuty meczu dla systemu GAP z odpowiednimi funkcjami kalkulacyjnymi
- Konwertuje parametry do formatu wymaganego przez `RatingFactory`

**Obsługiwane parametry:**
- **ELO**: `initial_rating` (domyślnie: 1500), `second_tier_coef` (domyślnie: 0.8)
- **GAP**: `match_attributes` z automatycznym mapowaniem funkcji kalkulacyjnych
- Wspólne: `matches_df`, `teams_df`, `first_tier_leagues`, `second_tier_leagues`
...
# Utworzenie systemów rankingowych
...
**Dostępne funkcje kalkulacyjne dla GAP:**
- `chances_home`: Suma corner kicks i shots on target dla gospodarzy
- `chances_away`: Suma corner kicks i shots on target dla gości
- `goals_home`: Liczba bramek zdobytych przez gospodarzy
- `goals_away`: Liczba bramek zdobytych przez gości
- `btts`: Wskaźnik czy obie drużyny strzeliły bramkę (0 lub 1)
