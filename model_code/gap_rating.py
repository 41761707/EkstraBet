...
class GapRating(rating_strategy.RatingStrategy):
    ...
    Args:
        ...
        match_attributes (dict): Słownik atrybutów meczów do śledzenia i ich kalkulatorów
    """
    ...
    def calculate_rating(self):
        """ Oblicza ranking Gap dla wszystkich drużyn na podstawie wyników meczów. """
    ...
    def calculate_match_rating(self, match):
        """ Oblicza ranking Gap dla pojedynczego meczu i aktualizuje powery drużyn.
        Args:
            match (pd.Series): Dane meczu zawierające informacje o drużynach, wynikach itp.
        Returns:
            dict: Słownik z nowymi powerami i średnimi golami dla drużyn gospodarzy i gości
        """
        ...
        for attribute in self.match_attributes:
            if attribute not in self.last_five_matches[match['home_team']]:
                self.last_five_matches[match['home_team']][attribute] = deque(maxlen=5)
            if attribute not in self.last_five_matches[match['away_team']]:
                self.last_five_matches[match['away_team']][attribute] = deque(maxlen=5)
        main_attribute = list(self.match_attributes.keys())[0] if self.match_attributes else None
        ...
        for attribute, calculator in self.match_attributes.items():
            # Oblicz wartość dla danego atrybutu
            value = calculator['calculator'](match)
            if match['home_team'] not in self.last_five_matches:
                self.last_five_matches[match['home_team']] = {}
            if match['away_team'] not in self.last_five_matches:
                self.last_five_matches[match['away_team']] = {}
            if 'home' in attribute:
                if attribute not in self.last_five_matches[match['home_team']]:
                    self.last_five_matches[match['home_team']][attribute] = deque(maxlen=5)
                self.last_five_matches[match['home_team']][attribute].append(value)
            if 'away' in attribute:
                if attribute not in self.last_five_matches[match['away_team']]:
                    self.last_five_matches[match['away_team']][attribute] = deque(maxlen=5)
                self.last_five_matches[match['away_team']][attribute].append(value)
        return {
    ...
    def gap_update_home_team(self, match, i_h_att, i_h_def, i_a_att, j_a_def, j_a_att, i_a_def, s_h, s_a, l, phi_1):
        ''' Aktualizuje powery drużyny gospodarzy po meczu.
        Args:
            match (pd.Series): Dane meczu
            i_h_att (float): Power ataku gospodarzy przed meczem
            i_h_def (float): Power obrony gospodarzy przed meczem
            i_a_att (float): Power ataku gości przed meczem
            j_a_def (float): Power obrony gości przed meczem
            j_a_att (float): Power ataku gości przed meczem
            i_a_def (float): Power obrony gospodarzy przed meczem
            s_h (float): Średnia goli gospodarzy z ostatnich 5 meczów
            s_a (float): Średnia goli gości z ostatnich 5 meczów
            l (float): Współczynnik uczenia się
            phi_1 (float): Znaczenie meczu domowego
        '''
    ...
    def gap_update_away_team(self, match, j_a_att, j_h_att, j_a_def, j_h_def, i_h_def, i_h_att, s_h, s_a, l, phi_2):
        ''' Aktualizuje powery drużyny gości po meczu.
        Args:
            match (pd.Series): Dane meczu
            j_a_att (float): Power ataku gości przed meczem
            j_h_att (float): Power ataku gospodarzy przed meczem
            j_a_def (float): Power obrony gości przed meczem
            j_h_def (float): Power obrony gospodarzy przed meczem
            i_h_def (float): Power obrony gospodarzy przed meczem
            i_h_att (float): Power ataku gospodarzy przed meczem
            s_h (float): Średnia goli gospodarzy z ostatnich 5 meczów
            s_a (float): Średnia goli gości z ostatnich 5 meczów
            l (float): Współczynnik uczenia się
            phi_2 (float): Znaczenie meczu wyjazdowego
        '''
    ...
    def print_rating(self):
        """
        Wyświetla aktualne powery drużyn oraz średnie gole z ostatnich 5 meczów.
        """
        ...
        for _, team in self.teams_df.iterrows():
            ...
            main_attr = list(self.match_attributes.keys())[0] if self.match_attributes else None
    ...
    def get_data(self):
        """Zwraca zaktualizowane DataFrame'y meczów i drużyn po obliczeniu rankingów Gap."""
