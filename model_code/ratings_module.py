...
class WinnerRating:
    ...
    def update_rating(self, team_rating, win_ppb, team_result, const, round, cards):
        ...
        return team_rating + const * (team_result - win_ppb) * relax * card
    ...
    def rating_wrapper(self):
        ...
        for index, _ in self.matches_df.iterrows():
            ...
            self.matches_df.at[index, 'home_rating'] = self.ratings[home_team]
            self.matches_df.at[index, 'away_rating'] = self.ratings[away_team]
...
class BTTSRating:
    ...
    def rating_wrapper(self):
        ...
        for index, _ in self.matches_df.iterrows():
            ...
            self.powers["{}a_att".format(self.matches_df.loc[index, 'away_team'])] = j_a_att
            self.powers["{}a_def".format(self.matches_df.loc[index, 'away_team'])] = j_a_def
            is_btts = 1 if int(self.matches_df.loc[index, 'home_team_goals']) > 0 and int(self.matches_df.loc[index, 'away_team_goals']) > 0 else 0
            self.last_five_matches[self.matches_df.loc[index, 'home_team']].append(is_btts)
            self.last_five_matches[self.matches_df.loc[index, 'away_team']].append(is_btts)
    def get_data(self):
        return self.matches_df, self.teams_df, self.teams_dict, self.powers, self.ratings, self.last_five_matches
