...
def insert_roster(cursor, api_call, club, external_ids, countries_dict, teams_dict, year_id):
        ...
        for player in players[current_key]:
            ...
            if player['id'] in external_ids:
                ...
                if int(player_old_team[1]) != int(teams_dict[club]):
                    ...
                    sql = '''INSERT INTO transfer(player_id, old_team_id, new_team_id, season_id) values ({}, {}, {}, {})'''.format(player_old_team[0], player_old_team[1], teams_dict[club], season)
                    #cursor.execute(query)
                    print(sql)
                    #UPDATE PLAYER
...
def get_players(conn, year_id):
    with conn.cursor() as cursor:
        query = "SELECT id, shortcut FROM teams WHERE sport_id = 2 and shortcut not in ('SEA')"
        cursor.execute(query)
        result = cursor.fetchall()
        ...
        for key, _ in teams_dict.items():
            ...
            insert_roster(cursor, get_roster, key, external_ids, countries_dict, teams_dict, year_id)
            #conn.commit()
            break
