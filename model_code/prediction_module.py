...
class PredictMatch:
    def __init__(self, matches_df, upcoming_df, teams_df, feature_columns, model, model_type, model_name, window_size, conn, prediction_automate: bool = False, model_config=None) -> None:
        """ Inicjalizacja klasy PredictMatch.
        Args:
            matches_df (pd.DataFrame): DataFrame z danymi historycznymi meczów.
            upcoming_df (pd.DataFrame): DataFrame z nadchodzącymi meczami do przewidzenia.
            teams_df (pd.DataFrame): DataFrame z informacjami o drużynach.
            feature_columns (list): Lista kolumn używanych jako cechy wejściowe do modelu.
            model: Wytrenowany model ML do przewidywania wyników meczów.
            model_type (str): Typ modelu ('winner', 'goals', 'btts', 'exact').
            model_name (str): Nazwa modelu w bazie danych.
            window_size (int): Długość sekwencji wejściowej dla modelu.
            conn: Połączenie z bazą danych.
            prediction_automate (bool): Flaga określająca, czy zapisywać predykcje do bazy danych.
            model_config (dict): Konfiguracja modelu zawierająca mapowania i inne ustawienia.
        """
    ...
    def get_additional_prediction_info(self):
        """ Pobiera dodatkowe informacje potrzebne do zapisu predykcji w bazie danych."""
        ...
        try:
            ...
            print(f"Bład przy pobieraniu ID: {str(e)}")
            return events, None
        finally:
            cursor.close()

    def get_sequences(self, team_id, game_date):
        """ Pobiera sekwencje danych dla danej drużyny przed określoną datą meczu.
        Args:
            team_id (int): ID drużyny.
            game_date (str): Data meczu w formacie 'YYYY-MM-DD'."""
        
        #TODO: usunać redundancję między tą klasą a process_data.py
        team_matches = self.matches_df[
            (((self.matches_df['home_team'] == team_id) 
            | (self.matches_df['away_team'] == team_id)) 
            & (self.matches_df['game_date'] < game_date))]
    ...
    def predict_match_period(self) -> None:
        #TODO: Tu chodzi o to, że jak wyżej możemy tylko jeden mecz przewidzieć
