...
Harmonogram: po imporcie wyników meczów oraz po batchu predykcji/kursów.
Uruchomienie „za wcześnie” jest bezpieczne (nie rozliczy meczu bez wyniku).

### Migracja indeksu (wymagana przed produkcyjnym `--write-db`)

Ręcznie (admin), skrypt `sql/model_statistics_maintenance.sql`:

1. Wykryj duplikaty automatycznych zakładów `(match_id, event_id, model_id)`.
2. Zastąp stary unikalny indeks indeksem `unique_model_bet(match_id, event_id, model_id)`.
3. Uruchom zapytania weryfikacyjne ze skryptu.

Aplikacja **nie** wykonuje DDL. Szczegóły tabel: [db_documentation](../db_documentation/db_documentation.md) (sekcje BETS, FINAL_PREDICTIONS).

### Smoke test (idempotencja)

1. Dry-run dla zakończonego meczu (`result` w `1/X/2`) i przyszłego — sprawdź raport.
2. `--write-db` (gdy indeks gotowy).
